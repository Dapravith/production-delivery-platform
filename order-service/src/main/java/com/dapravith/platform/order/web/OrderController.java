...
public class OrderController {
  ...
  Mono<Order> create(@AuthenticationPrincipal Jwt jwt, @Valid @RequestBody CreateOrder request) {
    return repository.save(new Order(UUID.randomUUID(), jwt.getSubject(), request.amount(), "CREATED", Instant.now()));
  }
  public record CreateOrder(@NotNull @DecimalMin(value = "0.01") BigDecimal amount) {}
}
